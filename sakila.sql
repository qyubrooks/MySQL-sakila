...
SELECT film.title, count(inventory.store_id) AS 'Total Inventory'
FROM film
...
SELECT customers.LastName, customers.FirstName, sum(amount) AS 'Total Paid'
FROM  customers
INNER JOIN payment ON payment.customer_id = customers.ID
...
SELECT title AS "Film Titles", count(rental.rental_id) AS "Rental_Times"
FROM film
...
SELECT store.store_id, sum(payment.amount) AS "Total Amounts"
FROM store 
...
SELECT category.name AS 'Top Five', sum(payment.amount) AS 'Gross Revenues'
FROM category
